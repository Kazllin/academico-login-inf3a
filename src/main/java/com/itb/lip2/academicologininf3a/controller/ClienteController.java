...
public class ClienteController {
 ...
 // ESSA URL VAI VIRAR UM BOTAO
	...
	public ResponseEntity<CarrinhoDto> getCarrinho(@PathVariable Long id) {
	    ...
	    for (ProdutoCarrinho produtoCarrinho : carrinho.getProdutos()) {
	        double valorProduto = produtoCarrinho.getProduto().getPreco() * produtoCarrinho.getQuantidade();
	        valorTotal += valorProduto; // Acumula o valor total

	        // Cria um DTO para o produto e adiciona à lista
	        ProdutoDto dto = new ProdutoDto();
	        dto.setId(produtoCarrinho.getProduto().getId());
	        dto.setNome(produtoCarrinho.getProduto().getNome());
	        dto.setQuantidadeDesejada(produtoCarrinho.getQuantidade());
	        dto.setValor(valorProduto);
	        
	        // Adiciona a imagem do produto ao DTO
	        String imagemPath = produtoCarrinho.getProduto().getImagem(); // Supondo que você tenha um método para obter a imagem
	        if (imagemPath != null && !imagemPath.isEmpty()) {
	            dto.setImagem(imagemPath); // Define a imagem no DTO
	        }
